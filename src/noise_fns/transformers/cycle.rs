...
#[inline(always)]
fn lerp(u: f64, g0: f64, g1: f64) -> f64 {
    let k0 = g0;
    let k1 = g1 - g0;
    k0 + k1 * u
}
...
/// use TranslatePoint.
///
/// Cycle may be useful for repeated tiled noise, or noise around a polar domain.
...
impl<Source> NoiseFn<[f64; 1]> for Cycle<Source>
where
...
impl<Source> NoiseFn<[f64; 3]> for Cycle<Source>
where
...
impl<Source> NoiseFn<[f64; 4]> for Cycle<Source>
where
...
mod tests {
    ...
    fn repeats_with_period_2d() {
        let source = Perlin::default();
        let cycle = Cycle::new(&source).set_x_period(10.).set_y_period(20.);
        assert!(approx_eq!(
            f64,
            cycle.get([5.1, 0.1]),
            cycle.get([15.1, 0.1])
